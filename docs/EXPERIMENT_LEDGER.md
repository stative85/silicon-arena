...
| **SWARM-F** | Does the blind swarm keep allocating when agents vanish, go silent, or return? | `59dd514`, amended `86959a9` and `a495bcb` | 120 matches, 4,800 allocations: **zero** malformed bids and **zero** fallback wakes across n=5/4/3, intermittent toggling and full removal-and-rejoin; dropout recovery 80/80; max silence never exceeded the F0 ceiling of 8; n=2 woke the fallback on 63.6% of turns against a pre-registered >=50%; **rejoin failed 4 times in 60 F3 events** | **PARTIAL** — allocation tolerance holds to a roster of three and stops at two exactly where the frozen arithmetic said; **reintegration is not guaranteed for absences under 8 turns**, because the returner only outbids a named survivor once starvation saturates. Statistic 3's bar was **mis-specified** (a no-naming baseline scored against live runs) and is demoted to descriptive; it established a stable +0.94 extra bidder from direct address ([EXPERIMENT_SWARM_F](EXPERIMENT_SWARM_F.md)) |
