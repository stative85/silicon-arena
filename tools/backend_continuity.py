...
from artifact_schema import SchemaRefused, load_checked, validate  # noqa: E402
...
# Set from --overwrite. Default refuses to replace an existing roll-up.
OVERWRITE = False
...
def selftest():
    ...
    # BC-5 TOOTH. The reader used to hard-index `d["samples"]`. Prove that it
    # now refuses by name instead. The four arm artifacts on disk are the real
    # test material: they are an older schema than the current writer emits and
    # declare no experiment_id, so every one of them must be refused. If this
    # check ever flips, either the artifacts were regenerated by a corrected
    # harness -- which would be news -- or the schema went soft.
    present = [a for a in ARMS
               if os.path.exists(os.path.join(RESULTS,
                                              "RUNTIME_MEMORY_%s.json" % a))]
    if present:
        docs, refused = read_arms()
        ck("undeclared arm artifacts refused, not KeyError'd",
           (sorted(refused), sorted(docs)), (sorted(present), []))
        ck("every refusal names the missing declaration",
...
def read_arms():
    """Load every present arm artifact through the schema, or refuse.

    BC-5: this function used to be `json.load` followed by `d["samples"]`. A
    file at the expected path that was not what it claimed produced a KeyError
    -- a stack trace, which is not a refusal, because a stack trace can be
    read as 'the tooling is flaky' instead of 'this evidence is not admissible'.
    Refusal is now explicit, names the arm, and is fatal for the whole run:
    a partial roll-up over the arms that happened to parse is a worse artifact
    than no roll-up at all.
    """
    docs, refused = {}, {}
    for arm in ARMS:
        ...
        if not os.path.exists(p):
            continue
        try:
            docs[arm] = load_checked(p, "RUNTIME_MEMORY_ARM")
...
def apply_to_arms():
    docs, refused = read_arms()
    if refused:
        print("=== REFUSED -- arm artifacts are not what they claim to be ===")
        for arm, why in refused.items():
            print("  %-17s %s" % (arm, why))
        print("\nNo roll-up written. The existing RM_BACKEND_CONTINUITY.json is")
        print("left untouched; a witness computed over unverified inputs is not")
        print("a witness. See docs/results/STATIC_AUDIT_RUNTIME_RECOVERY.md BC-5.")
        return 1
    if not docs:
        print("No arm artifacts present. Nothing to witness.")
        return 1
    ...
    for arm, d in docs.items():
        ...
        for k, v in r["clauses"].items():
            print("      %-30s %s" % (k, v))
    # THE ROLL-UP DECLARES ITSELF TOO. It used to be a bare {arm: result} map
    # with no statement of which experiment it belonged to or what produced it,
    # which is the exact defect this module now refuses in its own inputs.
    doc = {
        "artifact_kind": "RM_BACKEND_CONTINUITY",
        "experiment_id": "RUNTIME-MEMORY",
        "produced_by": "tools/backend_continuity.py --apply",
        "arms": out,
    }
    self_bad = validate(doc, "RM_BACKEND_CONTINUITY")
    if self_bad:
        print("\nREFUSING TO WRITE -- own output fails its own schema: %s"
              % "; ".join(self_bad))
        return 1

    dest = os.path.join(RESULTS, "RM_BACKEND_CONTINUITY.json")
    if os.path.exists(dest) and not OVERWRITE:
        print("\nREFUSING TO WRITE -- %s already exists." % os.path.basename(dest))
        print("Superseded evidence is amended, not overwritten. Re-invoke with")
        print("--overwrite only when a human has decided the existing roll-up")
        print("is safe to replace.")
        return 1
...
def main():
    ...
    ap.add_argument("--overwrite", action="store_true",
                    help="permit replacing an existing roll-up artifact")
    a = ap.parse_args()
    global OVERWRITE
